<template>
	<virtual-waterfall ref="waterfallRef" :fetchPage="$api.space.list" :getItemId="item => item._id"
		:columnCount="{ 1920: 5, 1280: 4, 1080: 3, 820: 2, 650: 1 }">
		<template #header>
			<div class="pageoperbar">
				<div class="pageoperbar-inner">
					<div class="pageoperbar-leftbar" @click="addNewItem">
		...
		<template #default="{ item, columnWidth, remove }">
			<flip-card :lockHeightOnFlip="true" :ref="el => el && flipCardMap.set(item._id, el)">
				<template #front="{ flip }">
					<div class="vw-crad" @click="navito(item)" @close="remove">
						<img v-if="item.cover" :src="item.cover" alt="" class="vw-crad-img" :style="{
							aspectRatio:
								`${item.originalWidth}/${item.originalHeight}`
						}" />
						<div class="vw-crad-body" :style="{...(!item.cover && { top: 0 }) }">
...
const navito = (item) => {
	router.push(`/folder/detail/${item._id}`)
}
...
.vw-crad {
	...
	&::before {
		...
		top: 0;
		bottom: 0;
		background: linear-gradient(to bottom, rgba(239, 74, 28, 0) 40%, rgba(0, 0, 0, 0.2) 100%);
	}
