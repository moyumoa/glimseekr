<template>
  <section ref="containerRef" data-masonry class="virtual-waterfall-container" :style="containerStyle" @scroll.passive="onScroll">
    ...
    <div class="virtual-waterfall-inner" :style="innerStyle">
      ...
      <section v-for="item in visibleItems" :key="item._id" :ref="el => measureItemIfNeeded(item._id, el)"
        :data-id="props.getItemId(item)" :class="['note-item', { 'newly-inserted': item._justInserted }]" :style="{
    ...
    <el-empty v-if="!isLoading && allItems.length === 0 && !isInitialLoading" description=" " style="margin-top: 20%" />
...
const positionedItems = computed(() => {
  ...
  return allItems.value.map(item => {
    ...
    const col = cols.indexOf(Math.min(...cols))  // 确保找到最短的列
    const top = cols[col]  // 计算当前项的 top 值
    const left = col * (columnWidth.value + HORIZONTAL_GAP)  // 计算当前项的 left 值
    cols[col] += height + VERTICAL_GAP  // 更新该列的高度
...
onMounted(() => {
  const resizeObserver = new ResizeObserver(([entry]) => {
    ...
    debouncedContainerWidth.value = rawContainerWidth.value;
    updateColumnCountByBreakpoint();  // 更新列数
    reMeasureVisibleHeights();  // 重新计算元素的尺寸
    onScroll();  // 确保滚动时能够正确加载更多
  });
